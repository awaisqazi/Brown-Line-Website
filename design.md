...
- **2026-07-02 (launch-prep pass: photo pipeline, splash realism, admin UX):** Full-site QA against the master doc plus three feature tracks.
  ...
  - **TrainSplash redesign (from owner-supplied platform photos of cars 3323/3426):** the splash train is now a photo-accurate CTA Brown Line railcar head-on: smooth gray-stainless face (no front fluting), center emergency door with window, safety chains, and car number `3323`; two windshields with the operator and parked wiper on the left; small amber destination sign top-center (dynamic next stop) with run box `417`; colored marker lenses (red/amber/green, blue/red) along the top band; paired round lamps low with red taillights outboard and white headlights inboard (beam cones re-anchored); ribbed anticlimber + coupler with air hoses. Removed the invented rainbow bumper stripes, cta roundel, and wide brown nose band.
