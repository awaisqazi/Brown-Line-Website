...
- Turnstile: a Cloudflare Turnstile widget ("The Brown Line", Managed mode, hostnames `thebrownlinechi.com` and `www.thebrownlinechi.com`) provides the keys. The public site key is set as the `PUBLIC_TURNSTILE_SITE_KEY` GitHub repo variable (consumed in `deploy.yml`) and is live on the production form. **However, as of 2026-07-02 `supabase secrets list` shows NO `TURNSTILE_SECRET` on the edge function**, so server-side verification is running on Cloudflare's always-pass TEST secret and the captcha is decorative; the function now logs a warning whenever the fallback is used. Set the real secret (Cloudflare dashboard → Turnstile → the widget's secret key) via `supabase secrets set TURNSTILE_SECRET=...` before launch. For local development the TEST-key fallback is intentional.
...
- **2026-07-02 (launch-prep pass: photo pipeline, splash realism, admin UX):** Full-site QA against the master doc plus three feature tracks.
  ...
  - **Known launch blockers (owner actions):** set `TURNSTILE_SECRET` (see §11); set `PUBLIC_GA_MEASUREMENT_ID` (no GA tag on the live site as of 2026-07-02); repoint the $7/mo Stripe link; the admin credential in the database no longer matches the ignored runbook (see `PRIVATE_README.md`); deploy the updated `submit-event` edge function.
