...
- Stop colors match the mobile drawer order: Maya Blue → Celadon → Baby Pink → Amber → Cayenne (Events, Submit an Event, Archive, About, Ethics). The `lg` `--route-gap` is tuned (5.25rem) so the longest label ("Submit an Event") clears its centered neighbors.
...
- **2026-06-30 (master-doc reconciliation pass):** Audited the whole site against the Website Master Change Doc and closed the genuinely-open items (most of the doc was already shipped in the 2026-06-25 rework, so its "still broken" claims were stale).
  ...
  - **Owner tweaks (same pass):** homepage terminus now reads "See all stops" and deep-links to `/events#all` (opens on All upcoming); nav labels changed "Submit" → "Submit an Event" and "Standards" → "Ethics" (route still points at `/standards`); widened the `lg` desktop route gap to 5.25rem so the longer "Submit an Event" label clears its neighbors.
